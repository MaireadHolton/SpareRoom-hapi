...
export const advertController = {
    ...
    makeAdvert: {
      handler: async function (request, h) {
        try {
          const loggedInUser = request.auth.credentials;
          await db.advertStore.makeAdvert(loggedInUser._id, request.payload.firstname, request.payload.college, request.payload.latitiude, request.payload.longitude, request.payload.rules, request.payload.price, request.payload.available);
